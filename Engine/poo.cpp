...
void Poo::Init(int in_x, int in_y, int in_vx, int in_vy) //constructor
{
...
void Poo::Update()
{
	...
	//clamps and bounce for poo x and y
	if (x <= 0)
	{
		x = 0;
		vx = -vx;
	...
	if ((y + h) >= Graphics::ScreenHeight)
	{
		y = Graphics::ScreenHeight - h;
		vy = -vy;
	}
}

//checks if colliding
void Poo::IsColliding(const Player& player)	//colliding with player
{
	if (
